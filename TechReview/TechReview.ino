...
// digital pins
    // inputs
int buttonSolenoid1 = 0; // [HENRY] solenoid 1 button pin
int buttonSolenoid2 = 1; // [HENRY] solenoid 2 button pin
int buttonStartStop = 2; // interrupt pin
int soapSensor = 9; 
int overflow = 10;
int buttonHeaterStartStop = 12; // [HENRY] heater start stop 
...
void setup() {
    ...
    pinMode(buttonSolenoid1, INPUT_PULLUP); // [HENRY] Solenoid 1 start/stop
    pinMode(buttonSolenoid2, INPUT_PULLUP); // [HENRY] Solenoid 2 start/stop
    pinMode(buttonHeaterStartStop, INPUT_PULLUP); // [HENRY] Heater start/stop
}
...
void loop() {
    ...
    if (pinHigh(bottleDetect) & pinHigh(doorDetect)){ // if bottle present and door closed 
        if (state == 1) {
            if (pinHigh(buttonStartStop)== false) state = 2; 
        }
        if (state ==2){
            if (cycle_time = 0){
                ON(doorLock); 
            ...
            if (pinHigh(buttonStartStop)== true) state = 4; 
            else Cycle();
        }
        if (state ==3){
            OFF(doorLock); 
